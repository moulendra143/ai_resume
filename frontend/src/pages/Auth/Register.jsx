...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await register(form);
      // Navigate to OTP verification page, passing email & password so we can auto-login after verify
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10">
      <GlassCard className="max-w-lg w-full">
        <h1 className="text-3xl font-semibold mb-2 text-white">Create your account</h1>
        <p className="text-slate-400 mb-8">Start using AI resume building, ATS analysis, and optimization tools.</p>
        ...
        <p className="mt-6 text-sm text-slate-400">
          Already registered? <Link className="text-violet-300 hover:text-white" to="/login">Sign in</Link>
        </p>
      </GlassCard>
    </div>
  );
